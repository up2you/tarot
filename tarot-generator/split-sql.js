...
// 配置
const INPUT_FILE = path.join(__dirname, 'output/relation_scenarios/all_relation_scenarios.sql');
const OUTPUT_DIR = path.join(__dirname, 'output/relation_scenarios/batches');
const BATCH_SIZE = 500; // 每個批次 500 筆資料

console.log('🚀 開始拆分 SQL 檔案...\n');
...
for (let i = 0; i < totalBatches; i++) {
    ...
    const outputFile = path.join(OUTPUT_DIR, `batch_${batchNumber}.sql`);
...
console.log('2. 依序複製 batch_01.sql, batch_02.sql... 的內容並執行');
console.log('3. 全部完成後執行驗證查詢');
