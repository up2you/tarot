        ...
        <div className="card-face card-front-side">
          {card && (
            <div className={`relative w-full h-full flex flex-col ${isReversed ? 'rotate-180' : ''}`}>
              ...
              {/* 牌名標籤 - 根據尺寸調整 */}
              <div className={`absolute left-1/2 -translate-x-1/2 w-[88%] transition-all duration-1000 ${isFlipped ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} ${size === 'xs' ? 'bottom-1' : size === 'sm' ? 'bottom-2' : 'bottom-6'}`}>
                <div className={`bg-[#fdfcf0]/90 backdrop-blur-md border border-[#d4af37] shadow-[0_15px_30px_rgba(0,0,0,1)] rounded-sm text-center ${size === 'xs' ? 'py-0.5 px-0.5' : size === 'sm' ? 'py-1 px-1' : 'py-2 px-1'}`}>
                  <p className={`text-black font-cinzel font-black leading-tight ${size === 'xs' ? 'text-[8px] tracking-[0.1em]' : size === 'sm' ? 'text-xs tracking-[0.1em]' : 'text-xl tracking-[0.2em]'}`}>{card.nameZh}</p>
                  {size !== 'xs' && (
                    <>
                      <div className="w-1/3 h-px bg-black/20 mx-auto my-1"></div>
                      <p className={`text-black/60 uppercase font-cinzel font-bold ${size === 'sm' ? 'text-[6px] tracking-[0.2em]' : 'text-[8px] tracking-[0.4em]'}`}>{card.name}</p>
                    </>
                  )}
                </div>
              </div>
