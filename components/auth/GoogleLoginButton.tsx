import React from 'react';

interface GoogleLoginButtonProps {
  onClick: () => void;
  isLoading?: boolean;
  disabled?: boolean;
  text?: string;
}

export const GoogleLoginButton: React.FC<GoogleLoginButtonProps> = ({
  onClick,
  isLoading = false,
  disabled = false,
  text = '使用 Google 登入',
}) => {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={disabled || isLoading}
      className={`w-full bg-white text-gray-800 font-medium text-sm sm:text-base py-3 px-4 rounded-xl flex items-center justify-center gap-3 shadow-md hover:bg-gray-50 active:scale-[0.98] transition-all duration-200 cursor-pointer ${
        disabled || isLoading ? 'opacity-70 cursor-not-allowed' : ''
      }`}
    >
      {/* Official Google G Logo SVG */}
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
        <path
          d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v4.51h6.6c-.29 1.52-1.14 2.82-2.4 3.68v3.05h3.88c2.27-2.09 3.665-5.17 3.665-9.17z"
          fill="#4285F4"
        />
        <path
          d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.88-3.05c-1.08.72-2.45 1.16-4.05 1.16-3.12 0-5.77-2.1-6.72-4.93H1.26v3.15C3.25 21.3 7.31 24 12 24z"
          fill="#34A853"
        />
        <path
          d="M5.28 14.27c-.25-.72-.38-1.49-.38-2.27s.13-1.55.38-2.27V6.58H1.26C.46 8.16 0 9.99 0 12s.46 3.84 1.26 5.42l4.02-3.15z"
          fill="#FBBC05"
        />
        <path
          d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42C17.95 1.19 15.24 0 12 0 7.31 0 3.25 2.7 1.26 6.58l4.02 3.15c.95-2.83 3.6-4.98 6.72-4.98z"
          fill="#EA4335"
        />
      </svg>
      <span>{isLoading ? '處理中...' : text}</span>
    </button>
  );
};
